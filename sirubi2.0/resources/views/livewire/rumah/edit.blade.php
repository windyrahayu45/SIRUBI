...
<script>
document.addEventListener('livewire:navigated', function () {
    console.log("✅ Livewire navigated, initialize Leaflet map");

    const latInput = document.getElementById('latitude');
    const lngInput = document.getElementById('longitude');

    if (!latInput || !lngInput) {
        console.warn("⚠️ Input latitude/longitude tidak ditemukan");
        return;
    }

    const defaultLat = -0.305441;
    const defaultLng = 100.369164;
    ...
    window._leaflet_marker = marker;

    // Update input & Livewire saat drag
...
<script>
document.addEventListener('livewire:navigated', function () {
    console.log("✅ Livewire navigated, listener aktif");

    // 🔥 Listener untuk event dari PHP
    window.addEventListener('setAllSelect2', (e) => {
        console.log("🔥 Event browser diterima:", e.detail);

        // Update semua Select2
        Object.entries(e.detail).forEach(([key, value]) => {
            const el = document.getElementById(key);
            if (el) {
                $(el).val(value).trigger('change.select2');
                console.log(`✅ ${key} di-set ke ${value}`);
            }
        });
        ...
        const latInput = document.getElementById('latitude');
        const lngInput = document.getElementById('longitude');
        if (window._leaflet_map && latInput && lngInput) {
            const lat = parseFloat(latInput.value);
            const lng = parseFloat(lngInput.value);
            if (!isNaN(lat) && !isNaN(lng)) {
                if (window._leaflet_marker) {
                    window._leaflet_marker.setLatLng([lat, lng]);
                } else {
                    window._leaflet_marker = L.marker([lat, lng], { draggable: true }).addTo(window._leaflet_map);
                }
                window._leaflet_map.setView([lat, lng], 15);
                console.log(`📍 Marker update ke [${lat}, ${lng}]`);
            }
        }
    });
});
